...
;(async () => {
  ...
  for (const [slug, nombre] of [['norte', 'Agua Norte'], ['sur', 'Agua Sur']]) {
    const r = await req('POST', '/plataforma/distribuidoras', { token: P, body: { slug, nombre, colorPrimario: '#112233', superadmin: { username: 'jefe', password: 'clave-segura-1' } } })
    ok(r.status === 201, `alta distribuidora ${slug} (mismo usuario "jefe" en ambas)`)
  }
  ok((await req('POST', '/plataforma/distribuidoras', { token: P, body: { slug: 'norte', nombre: 'x', superadmin: { username: 'a', password: '12345678' } } })).status === 409, 'slug repetido → 409')
  ok((await req('POST', '/plataforma/distribuidoras', { token: P, body: { slug: 'Mal Slug!', nombre: 'x', superadmin: { username: 'a', password: '12345678' } } })).status === 400, 'slug inválido → 400')
  ...
  ok(ln.status === 200 && ls.status === 200, 'login superadmin en cada distribuidora')
  ...
  const fcfg = await req('PUT', '/fidelidad/config', { token: N, body: { activo: true, productoPremioId: ps.json.producto.id } })
  ok(fcfg.status === 400, `premio con producto de otra distribuidora → 400 (${fcfg.status})`)
  const fcfg2 = await req('PUT', '/fidelidad/config', { token: N, body: { activo: true, productoPremioId: pn.json.producto.id, sellosParaPremio: 5 } })
  ...
  ok((await req('PUT', '/contenido', { token: N, body: { hero: 'Hola norte' } })).status === 200, 'contenido guardado')
  const contS = await req('GET', '/contenido', { slug: 'sur' })
  ok(JSON.stringify(contS.json) === '{}', 'sur no ve el contenido de norte')
  const aj = await req('PUT', '/distribuidora/ajustes', { token: N, body: { nombre: 'Agua Norte S.A.', whatsapp: '0999' } })
  ok(aj.status === 200 && aj.json.distribuidora.nombre === 'Agua Norte S.A.', 'superadmin cambia su marca')
